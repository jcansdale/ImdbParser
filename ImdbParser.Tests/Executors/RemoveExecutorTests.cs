...
namespace ImdbParser.Tests.Executors {
    /// <summary><see cref="RemoveExecutor" /> tests</summary>
    [ExcludeFromCodeCoverage]
    static class RemoveExecutorTests {
#region Constructor tests
        /// <summary><see cref="RemoveExecutor(ISettings)" /> test.</summary>
        [Test]
        public static void Constructor_ValidArgumentPassed_NoExceptionThrown() => That(new RemoveExecutor(Setup().settings.Object), Not.Null);
#endregion

#region ExecuteAsync tests
        /// <summary><see cref="RemoveExecutor.ExecuteAsync" /> test.</summary>
        ...
        public static async Task ExecuteAsync_NoDirectories_SuccessReturned() {
            ...
            (await setup.executor.ExecuteAsync(CreateRemove()).ConfigureAwait(default)).ExpectSuccess();
        }
        ...
        /// <returns><see cref="Task" /> to execute.</returns>
        [Test]
        public static async Task ExecuteAsync_FileFoundAndDirectoryNotEmpty_DirectoryNotDeleted() {
            var setup = Setup();
            var subDirectory = new Mock<IDirectoryInfo>();
            setup.settings.SetupSettings().SetupDirectoryEnumerateDirectories(subDirectory.Object);
            subDirectory.SetupDirectoryEnumerateFiles(new Mock<IFileInfo>().Object);
            subDirectory.SetupDirectoryEnumerateFileSystemInfos(new Mock<IFileSystemInfo>().Object);

            (await setup.executor.ExecuteAsync(CreateRemove()).ConfigureAwait(default)).ExpectSuccess();

            subDirectory.Verify(self => self.Delete(), Never);
        }

        /// <summary><see cref="RemoveExecutor.ExecuteAsync" /> test.</summary>
        /// <returns><see cref="Task" /> to execute.</returns>
        [Test]
        public static async Task ExecuteAsync_FileFoundAndDirectoryEmpty_DirectoryDeleted() {
            var setup = Setup();
            var subDirectory = new Mock<IDirectoryInfo>();
            setup.settings.SetupSettings().SetupDirectoryEnumerateDirectories(subDirectory.Object);
            subDirectory.SetupDirectoryEnumerateFiles(new Mock<IFileInfo>().Object);
            ...
            subDirectory.Verify(self => self.Delete(), Once);
        }
#endregion

        /// <summary>Single test setup.</summary>
        /// <returns><see cref="object" /> to test with its <see cref="Mock" />.</returns>
        static (RemoveExecutor executor, Mock<ISettings> settings) Setup() {
            var settings = new Mock<ISettings>();
            return (new RemoveExecutor(settings.Object), settings);
        }
        ...
        /// <summary>Sets up <paramref name="directory" /> <see cref="IDirectoryInfo.EnumerateFileSystemInfos" /> to return <paramref name="infos" />.</summary>
        /// <param name="directory"><see cref="IDirectoryInfo" /> <see cref="Mock" /> to set up.</param>
        /// <param name="infos">Items to return from <paramref name="directory" /> <see cref="IDirectoryInfo.EnumerateFileSystemInfos" />.</param>
        static void SetupDirectoryEnumerateFileSystemInfos(this Mock<IDirectoryInfo> directory, params IFileSystemInfo[] infos) =>
            directory.Setup(self => self.EnumerateFileSystemInfos()).Returns(infos);
